...
class findCaptchas(baseDiscoveryPlugin):
    ...
    def _get_images(self, fuzzable_request):
        ...
        try:
            response = self._url_opener.GET(fuzzable_request.getURI(), useCache=False)
        except:
            om.out.debug('Failed to retrieve the page for finding captchas.')
        else:
            try:
                # Better not use DPCache here, it's no good.
                document_parser = DocumentParser(response)
                ...
                for img_src in image_list:
                    ...
                    try:
                        image_response = self._url_opener.GET(img_src, useCache=False)
                    except:
